...
function copyFolder(sourceFolder,targetFolder){
  ...
  Logger.log("Done with the folder " + sourceFolder.getName() + " total " + totalFiles + " copied")
  if(totalFiles >= maxFilesSoft){
    Logger.log("Exiting because we have reached the limit, schedule to run again in " + rescheduleSeconds);
    removeTriggers();
    ScriptApp.newTrigger("start")
      .timeBased()
...
function copyFiles(sourceFolder,tFolder){
  ...
  while(files.hasNext()) {
    ...
    if(fileExists(file.getName(), tFolder.getId())){
      ...
      Logger.log("Copying file " + file.getName() + " to folder "  + tFolder.getName());
